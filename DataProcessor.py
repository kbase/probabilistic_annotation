...
#
# For now I just assign this as the MAXIMUM for each role
# to avoid diluting out by noise.
#
# Having the one with the actual genes in there is still useful for
# diagnosis and for annotation later!
#
...
def ComplexProbabilities(organismid, total_role_probability_file):
    ...
    for cplx in complexesToRequiredRoles:
        ...
        if len(noexistRoles) == len(allCplxRoles):
            TYPE = "CPLX_NOREPS"
            fid.write("%s\t%1.4f\t%s\t%s\t%s\n" %(cplx, 0.0, TYPE, SEPARATOR.join(unavailRoles), SEPARATOR.join(noexistRoles)))
            continue
        ...
        fid.write("%s\t%1.4f\t%s\t%s\t%s\n" %(cplx, minp, TYPE, SEPARATOR.join(unavailRoles), SEPARATOR.join(noexistRoles)))
...
def ReactionProbabilities(organismid, complex_probability_file):
    ...
    # Cplx --> {Probability, type}
    cplxToTuple = {}
    for line in open(complex_probability_file, "r"):
        spl = line.strip("\r\n").split("\t")
        cplxToTuple[spl[0]] = ( float(spl[1]), spl[2] )

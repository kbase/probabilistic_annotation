...
class ProbabilisticAnnotationWorker:
    ...
    def __init__(self, genomeId, jobId=None, context=None):
        ''' Initialize object.

            @param genomeId: Genome ID string for genome being annotated
            @param jobId: Job ID for job running the worker
        ...
        serviceName = os.environ.get('KB_SERVICE_NAME', ServiceName)
        cfg = ConfigParser()
    ...
    def skippedFeatures(self, features, blastResultFile, queryToRolesetProbs):
        ''' Create the object data for a ProbAnno typed object.
            ...
            @return List of skipped feature IDs
        '''

        self._log(log.DEBUG, 'Started finding skipped features for genome %s...' %(self.genomeId))
        ...
        # Run through the list of features and see if there are any that did not get a hit.
        skippedFeatures = list()
        for index in range(len(features)):
            queryid = features[index]['id']
            ...
            if queryid not in queryToRolesetProbs or queryid not in queryToTargetEvals:
                skippedFeatures.append(queryid)
                
        self._log(log.DEBUG, 'Finished finding skipped features for genome %s' %(self.genomeId))
...
#            partialGprList = [ "(" + s + ")" for s in [ rolesToGeneList[f] for f in availRoles ] ]
    ...
    def cleanup(self):
        ...
        if self.workFolder is not None:
            if self.logger.get_log_level() < log.DEBUG2:
                try:
                    shutil.rmtree(self.workFolder)
                    os.rmdir(self.workFolder)
                except OSError:
                    ...
                    msg = 'Unable to delete temporary directory %s %s' %(self.workFolder, e)
